...
@pytest.mark.asyncio
async def test_generate_scaffolding_success(mock_llm_bridge, mock_knowledge_service, mock_model_id):
    # Setup
    service = GenaiScaffoldingService(mock_llm_bridge, mock_knowledge_service)
    ...
    # Verify invocations
    assert mock_llm_bridge.generate.call_count == 3
    
    # Verify Knowledge Service called
    mock_knowledge_service.get_architecture_guidelines.assert_called_once()

@pytest.mark.asyncio
async def test_generate_scaffolding_planning_failure(mock_llm_bridge, mock_knowledge_service, mock_model_id):
    # Setup
    service = GenaiScaffoldingService(mock_llm_bridge, mock_knowledge_service)
