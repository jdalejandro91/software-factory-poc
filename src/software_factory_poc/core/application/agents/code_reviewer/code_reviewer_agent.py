...
class CodeReviewerAgent(BaseAgent):
    ...
    async def _run_deterministic(self, mission: Mission) -> None:
        ...
        try:
            ...
            logger.error("[Reviewer] Critical error for %s: %s", mission.key, e, exc_info=True)
    ...
    async def _report_failure(self, mission_key: str, error: Exception) -> None:
        try:
            await self._tracker.add_comment(
                mission_key, f"Error ejecutando revision automatica: {error}"
            )
        except Exception as report_err:
            logger.error("[Reviewer] Failed to report error to Jira: %s", report_err)
