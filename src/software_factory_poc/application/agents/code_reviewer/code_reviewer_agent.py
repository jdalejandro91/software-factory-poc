...
from software_factory_poc.application.agents.code_reviewer.contracts.code_reviewer_contracts import CodeReviewResponseSchema
...
class CodeReviewerAgent(BaseAgent):
    ...
    async def execute_flow(self, mission: Mission) -> None:
        ...
        try:
            ...
            review_schema: CodeReviewResponseSchema = await self.brain.generate_structured(
                prompt=full_prompt,
    ...
    def _extract_mr_iid(self, mr_url: str) -> str:
        ...
        # Si ya es un numero directo, retornarlo tal cual
    ...
    def _to_domain_report(self, schema: CodeReviewResponseSchema) -> CodeReviewReport:
        ...
        Aplica las invariantes del dominio:
        - Si hay issues CRITICAL, el MR no puede ser aprobado (forzado por CodeReviewReport.__post_init__).
        - ReviewSeverity valida que el severity sea uno de los valores permitidos.
        """
        domain_comments = [
            ReviewComment(
        ...
        # El dominio fuerza rechazo si hay CRITICAL issues (invariante de CodeReviewReport)
