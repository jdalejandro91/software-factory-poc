...
from software_factory_poc.application.core.entities.artifact_result import ArtifactResultModel
from software_factory_poc.infrastructure.observability.logger_factory_service import build_logger

logger = build_logger(__name__)


class RunResultStoreFileAdapter(Repository[ArtifactResultModel]):
    ...
    def find_by_id(self, id: str) -> ArtifactResultModel | None:
        ...
        if item:
            try:
                ...
                logger.error(f"Failed to parse stored result for {id}: {e}")
                return None
        return None

    def save(self, entity: ArtifactResultModel) -> None:
        data = self._read_json()
        # Serialize model to dict
        data[entity.run_id] = entity.model_dump(mode='json')
        self._write_json(data)

    def find_all(self) -> list[ArtifactResultModel]:
        data = self._read_json()
        results = []
        for k, v in data.items():
            try:
               results.append(ArtifactResultModel(**v))
            except Exception:
               pass
        return results
