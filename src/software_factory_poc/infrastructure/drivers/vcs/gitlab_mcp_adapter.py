...
class GitlabMcpAdapter(VcsDriver):
    ...
    async def validate_branch_existence(self, branch_name: str) -> bool:
        ...
        try:
            response = await self._call_mcp("gitlab_get_branch", arguments={
                "project_id": self.project_id,
                "branch": branch_name,
            })
            # Si la llamada no falla, la rama existe
            return True
        except ProviderError:
            # Si falla (404 o similar), la rama no existe
            return False
    ...
    async def create_branch(self, branch_name: str, ref: str = "main") -> str:
        ...
        response = await self._call_mcp("gitlab_create_branch", arguments={
            "project_id": self.project_id,
            "branch": branch_name,
            "ref": ref,
        })
    ...
    async def create_merge_request(
        ...
        response = await self._call_mcp("gitlab_create_merge_request", arguments={
            "project_id": self.project_id,
